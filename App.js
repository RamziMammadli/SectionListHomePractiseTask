...
const App = () => {
  ...
  Info.forEach((item) => {
    let newDataItem = data.find((q) => q.title == item.firstName.charAt(0).toUpperCase())

    if(newDataItem){
      newDataItem.data.push(item.firstName, item.lastName, item.number)
    } else {
      let itemSample = {
        title: item.firstName.charAt(0).toUpperCase(),
        data: [item.firstName, item.number]
    }
    ...
    <>
      <SectionList
        ...
        renderSectionHeader={({ section: { title } }) => (
          <Text style={{ fontSize: 55, paddingLeft:20 }}>{title}</Text>
        )}
      />
    </>
  )}
